...
//
// The four SDKs version independently -- a fix to the Swift client is not a
// reason to bump the browser one -- so a bare `v0.2.0` is ambiguous and is
// rejected. The manifest check exists because `npm version` writes package.json
...
const PACKAGES = {
  ...
  swift: { dir: 'sdk/swift', kind: 'swiftpm', name: 'AgentRay' },
};
...
const m = /^(browser|server|python|swift)-v(\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?)$/.exec(tag);
if (!m) {
  console.error(`unrecognised release tag: ${tag}`);
  console.error('expected <browser|server|python|swift>-v<semver>, e.g. browser-v0.2.0');
  console.error('(a bare v0.2.0 is a product tag, not an SDK release -- the four SDKs version independently)');
  process.exit(1);
}
...
// SwiftPM resolves versions from git tags, so for swift the tag *is* the
// manifest -- there is nothing to cross-check against.
