...
| Memory curation fence | `memory_edit`/`learn` write only the running agent's own scope: the scope comes from `RunInfo.ScopeID`, never from tool args, and the store refuses an id outside it. Retraction is soft-supersede only — no hard delete — and both tools are ordinary gated tools (policy must permit them; absent without a store). | `agentcore/plugins/memory`, `agentcore.MemoryCurator` |
