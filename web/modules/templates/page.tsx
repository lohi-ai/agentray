        ...
        <div className="grid grid-cols-3 gap-4 max-[980px]:grid-cols-1">
          {templates.map((t) => (
            <div className="rounded-xl bg-[var(--color-background-card)] p-4" key={t.id}>
              <div className="flex items-center gap-2 mb-3"><h3 className="m-0 text-sm font-semibold">{t.name}</h3>{t.is_system ? <span className="rounded-full bg-[var(--color-background-muted)] px-2 py-0.5 text-2xs font-medium uppercase tracking-[0.08em] text-[var(--color-text-secondary)]">system</span> : null}</div>
