...
// One dialog for both "Add provider" and "Replace key" — the only difference is
...
export function ProviderDialog({
  ...
  busy: boolean;
  onSubmit: (input: WorkspaceProviderInput) => void;
  onClose: () => void;
}) {
  ...
  const submit = () => {
    ...
    onSubmit({
      ...
      // (runtime.go resolveBaseURL) — so without this gate an abandoned gateway
      // URL silently becomes the endpoint the new OpenAI key talks to.
  ...
  return (
    <Modal
      title={editing ? `Replace key — ${provider?.name || vendorLabel(provider?.vendor ?? '')}` : 'Add AI provider'}
      onClose={onClose}
      footer={
        <>
