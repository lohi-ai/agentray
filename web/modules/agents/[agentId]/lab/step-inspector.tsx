  ...
  return (
    <div className="flex flex-col gap-3.5">
      ...
      <Section title="Summary kept in place of the dropped span">
        <p className="m-0 whitespace-pre-wrap break-words text-sm leading-[1.55] text-[var(--color-text-primary)]">{step.summary || '—'}</p>
      </Section>
  ...
  return (
    <div className="flex flex-col gap-3.5">
            ...
            <div className="flex flex-col gap-1.5">
              {skillsAdvertised.map((s) => (
                <div className="flex items-baseline gap-[9px]" key={s.id}>
                  ...
                  <span className="text-xs leading-[1.45] text-[var(--color-text-secondary)]">{s.description}</span>
                </div>
              ))}
            </div>
      ...
      <Section title="Assistant response">
        <ConceptCaption concept="loop" />
        <p className="m-0 whitespace-pre-wrap break-words text-sm leading-[1.55] text-[var(--color-text-primary)]">{step.response || '—'}</p>
      </Section>
    </div>
  );
}
