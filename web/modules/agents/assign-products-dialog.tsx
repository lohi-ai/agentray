...
export function AssignProductsDialog({ agentID, agentName, onClose }: { agentID: string; agentName: string; onClose: () => void }) {
  ...
  return (
    ...
    >
      <p style={{ color: 'var(--muted-foreground)', fontSize: 'var(--fs-sm)', marginTop: 0 }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          {projects.length === 0 ? (
            <p style={{ fontSize: 'var(--fs-sm)', color: 'var(--muted-foreground)' }}>No other products in this workspace yet.</p>
          ) : null}
        </div>
      )}
    </Modal>
