...
func TestFormatAgentError(t *testing.T) {
	if got := formatAgentError("agent is disabled for this project"); !strings.Contains(got, "Set up") {
		t.Fatalf("disabled → %q", got)
	}
	if got := formatAgentError("no workspace model key configured"); !strings.Contains(got, "Settings") {
		t.Fatalf("no key → %q", got)
	}
	if got := formatAgentError("boom"); got != "boom" {
