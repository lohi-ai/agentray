...
// scopes (data_quality + analyze_build — owned by workloads) resolve to the
// run_sql + chart authoring tools through the same policy a human-configured
// agent uses. Catalog content is tested in internal/workloads; this test pins
// the policy seam.
func TestDataAnalystPresetGrantsSQLTools(t *testing.T) {
	granted := map[string]bool{}
	for _, n := range ScopeToolNames(ScopesFromMap(map[string]bool{
		"data_quality":  true,
		"analyze_build": true,
	})) {
		granted[n] = true
