...
func TestCaptureLiftsUTMTagsIntoColumns(t *testing.T) {
	...
	ev := events.got[0]
	if ev.UTMSource != "newsletter" || ev.UTMMedium != "email" || ev.UTMCampaign != "launch-week" {
		t.Fatalf("utm columns = %q/%q/%q, want newsletter/email/launch-week", ev.UTMSource, ev.UTMMedium, ev.UTMCampaign)
	}
	...
	rec = postCapture(t, h, "/capture", `{
		...
		"properties": {"path": "/pricing", "utm_source": "google", "utm_campaign": "brand"}
	}`)
	if rec.Code != http.StatusOK {
		t.Fatalf("bare-key capture: %d %s", rec.Code, rec.Body.String())
	}
	ev = events.got[1]
	if ev.UTMSource != "google" || ev.UTMCampaign != "brand" || ev.UTMMedium != "" {
		t.Fatalf("bare utm columns = %q/%q/%q, want google//brand", ev.UTMSource, ev.UTMMedium, ev.UTMCampaign)
	}
	...
	ev = events.got[2]
	if ev.UTMSource != "" || ev.UTMMedium != "" || ev.UTMCampaign != "" {
		t.Fatalf("untagged utm columns = %q/%q/%q, want all empty", ev.UTMSource, ev.UTMMedium, ev.UTMCampaign)
	}
}
