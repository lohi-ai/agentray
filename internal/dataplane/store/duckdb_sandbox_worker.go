...
type sandboxRequest struct {
	...
	Table string  `json:"table"` // delete | insert; cursor picks which table's cursor is read
	Rows  [][]any `json:"rows"`  // insert
	// Replace makes an insert an upsert. The landing table is re-copied
	// incrementally, and a row re-landed upstream keeps its key while moving to a
	// fresh synced_at, so the child must overwrite its copy of that key rather
	// than trip the primary key.
	Replace bool   `json:"replace"` // insert
...
type sandboxResponse struct {
	...
	// actually is rather than from where the parent assumes it is. The
	// Watermark* string fields complete the connector-row cursor, whose key is
	// the (synced_at, connector_id, table_name, row_key) tuple.
	Count       int64     `json:"count"`
	Watermark   time.Time `json:"watermark"`
	WatermarkID string    `json:"watermark_id"`
	// WatermarkConnector/WatermarkTable/WatermarkRowKey are the remaining three
	// parts of the landing table's high-water tuple.
	WatermarkConnector string `json:"watermark_connector"`
	WatermarkTable     string `json:"watermark_table"`
	WatermarkRowKey    string `json:"watermark_row_key"`
}
...
func (e *sandboxEngine) handle(ctx context.Context, req sandboxRequest) sandboxResponse {
	...
	case "cursor":
		...
		defer e.mu.Unlock()
		switch req.Table {
		case "", DuckDBEventsName:
			return e.eventsCursor(ctx)
		case DuckDBExternalRowsName:
			return e.externalRowsCursor(ctx)
		default:
			return errResponse(SandboxKindChild, fmt.Sprintf("unknown sandbox cursor table %q", req.Table))
		}
	case "delete":
	...
	case "insert":
		...
		e.mu.Lock()
		defer e.mu.Unlock()
		if err := e.insertRows(ctx, req.Table, req.Rows, req.Replace); err != nil {
			return errResponse(SandboxKindChild, err.Error())
		}
		return sandboxResponse{Kind: "ok"}
	case "query":
...
// drift (retention deleting upstream) is visible as child > main.
...
// externalRowsCursor reports the greatest (synced_at, connector_id, table_name,
// row_key) tuple this instance holds for the landing table. The tuple is read
// as a row rather than as per-column maxima, so the range predicate the parent
// resumes from never skips a row that shares the high-water timestamp. Count is
// the child's row total, which the parent compares against the main file both
// before its copy (deletion-only drift) and after it (a delete a landing
// masked).
func (e *sandboxEngine) externalRowsCursor(ctx context.Context) sandboxResponse {
	resp := sandboxResponse{Kind: "ok"}
	var watermark sql.NullTime
	var connector, table, key sql.NullString
	err := e.feeder.QueryRowContext(ctx,
		`SELECT synced_at, connector_id::VARCHAR, table_name, row_key FROM external_rows
		 ...
		 LIMIT 1`).Scan(&watermark, &connector, &table, &key)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return errResponse(SandboxKindChild, err.Error())
	}
	// An empty table leaves the zero tuple: the parent's range predicate then
	// matches every row, which is what a cold copy wants.
	resp.Watermark = watermark.Time
	resp.WatermarkConnector, resp.WatermarkTable, resp.WatermarkRowKey = connector.String, table.String, key.String
	if err := e.feeder.QueryRowContext(ctx, `SELECT count(*) FROM external_rows`).Scan(&resp.Count); err != nil {
		return errResponse(SandboxKindChild, err.Error())
	}
	return resp
}
...
// multi-row INSERT the in-process copy used. replace turns that statement into
// INSERT OR REPLACE, which the incremental landing-table copy needs: the child
// holds the key already when a row is re-landed upstream with a fresh
// synced_at, so a plain insert would trip the primary key.
func (e *sandboxEngine) insertRows(ctx context.Context, table string, rows [][]any, replace bool) error {
	if len(rows) == 0 {
		return nil
	}
	verb := "INSERT INTO"
	...
	_, err := e.feeder.ExecContext(ctx, fmt.Sprintf("%s %s VALUES %s", verb, table, batchPlaceholder), flat...)
	return err
}
