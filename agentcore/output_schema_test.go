...
func verdictSchema() *OutputSchema {
	return &OutputSchema{
		Name: "verdict",
...
// TestOpenAIEncodesOutputSchema pins the OpenAI wire mapping: OutputSchema →
...
func TestOpenAIEncodesOutputSchema(t *testing.T) {
	p := &OpenAIProvider{}
	raw, err := json.Marshal(p.encode(ChatRequest{Model: "m", OutputSchema: verdictSchema()}))
	if err != nil {
		t.Fatal(err)
	}
	body := string(raw)
	...
	strict.Strict = true
	raw, _ = json.Marshal(p.encode(ChatRequest{Model: "m", OutputSchema: strict}))
	if !strings.Contains(string(raw), `"strict":true`) {
		t.Fatalf("opt-in schema must emit strict:true: %s", raw)
	}

	raw, _ = json.Marshal(p.encode(ChatRequest{Model: "m"}))
...
func TestAnthropicEncodesOutputSchema(t *testing.T) {
	...
	if got := antBetaHeader(ChatRequest{Model: "m"}); got != "" {
		t.Fatalf("no betas must mean no header, got %q", got)
	}
}

// TestLoopThreadsOutputSchema verifies Config.OutputSchema reaches every
// provider request the loop issues.
func TestLoopThreadsOutputSchema(t *testing.T) {
	provider := NewFauxProvider(AssistantText(`{"verdict":"ok"}`))
