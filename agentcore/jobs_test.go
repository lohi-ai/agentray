...
func testJobsPolicy(t *testing.T, owner string) *jobsPolicy {
	t.Helper()
	p := newJobsPolicy(&JobSettings{}, owner, DefaultLimits())
	if p == nil {
...
func launcherFor(p *jobsPolicy, ctx context.Context) JobLauncher {
	return JobLauncher{store: p.store, owner: p.owner, base: ctx, maxBytes: p.maxBytes}
}
...
func TestJobs_FencedByOwner(t *testing.T) {
	...
	mine := &jobsPolicy{store: store, owner: "sess-1", maxWait: time.Second, maxBytes: 1024}
	theirs := &jobsPolicy{store: store, owner: "sess-2", maxWait: time.Second, maxBytes: 1024}
...
func TestJobs_ResultIsBoundedBeforeItReachesTheModel(t *testing.T) {
	p := newJobsPolicy(&JobSettings{MaxResultBytes: 200}, "sess-1", DefaultLimits())
	l := launcherFor(p, context.Background())
...
func TestJobs_WaitReturnsWhileStillRunningWithoutCancelling(t *testing.T) {
	p := newJobsPolicy(&JobSettings{MaxWaitSeconds: 1}, "sess-1", DefaultLimits())
	l := launcherFor(p, context.Background())
	release := make(chan struct{})
...
func TestJobs_FromContext(t *testing.T) {
	if _, ok := JobsFrom(context.Background()); ok {
...
func TestJobs_NilSettingsDisableTheSeam(t *testing.T) {
	if p := newJobsPolicy(nil, "sess-1", DefaultLimits()); p != nil {
		t.Fatal("nil settings must leave jobs off")
	}
}

func TestJobs_NoticeOnByDefault(t *testing.T) {
	if p := newJobsPolicy(&JobSettings{}, "s", DefaultLimits()); !p.notify {
		t.Fatal("completion notices must be on by default — polling is a turn tax")
	}
	if p := newJobsPolicy(&JobSettings{SuppressCompletionNotice: true}, "s", DefaultLimits()); p.notify {
