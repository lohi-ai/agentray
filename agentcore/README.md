...
| [`image.go`](image.go) | **loop** — normalizes every image part a `RichTool` returns before it crosses the provider seam: decode, bound dimensions and bytes, re-encode (PNG/JPEG), and a dimension note so the model knows what it lost. Sits in the kernel because the byte/dimension budget is the loop's own bounding — a plugin cannot be trusted to enforce it on itself. |
